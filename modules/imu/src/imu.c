...
int asr_imu_update(void)
{
	...
	for (int i = 0; i < 3; i++) {
		unit_status.imu_float_data.accel[i] =
			sensor_value_to_float(&sample.accel[i]);
		unit_status.imu_float_data.gyro[i] =
			sensor_value_to_float(&sample.gyro[i]);
	}
	unit_status.imu_float_data.temperature =
		sensor_value_to_float(&sample.temp);
	k_mutex_unlock(&unit_status_mutex);

	LOG_INF("accel [m/s2]: X=%10.4f  Y=%10.4f  Z=%10.4f",
		(double)unit_status.imu_float_data.accel[0],
		(double)unit_status.imu_float_data.accel[1],
		(double)unit_status.imu_float_data.accel[2]);
	LOG_INF("gyro [rad/s]: X=%10.4f  Y=%10.4f  Z=%10.4f",
		(double)unit_status.imu_float_data.gyro[0],
		(double)unit_status.imu_float_data.gyro[1],
		(double)unit_status.imu_float_data.gyro[2]);
	LOG_INF("die temp: %.1f C",
		(double)unit_status.imu_float_data.temperature);
	LOG_INF("------------------------------------------");
