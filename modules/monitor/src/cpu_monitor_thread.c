...
static void cpu_mon_thread_entry(void *p1, void *p2, void *p3)
{
	...
	k_timer_start(&cpu_mon_timer, K_MSEC(CPU_MON_PERIOD_MS),
		       K_MSEC(CPU_MON_PERIOD_MS));
...
{
	if (cpu_mon_started) {
		LOG_WRN("CPU monitor thread already started");
		return -EALREADY;
	}

	LOG_INF("==========================================");
	LOG_INF("ASR CPU monitor thread");
	LOG_INF("==========================================");
	LOG_INF("report interval: %u ms", CPU_MON_PERIOD_MS);
	LOG_INF("");

	k_thread_create(&cpu_mon_thread_data, cpu_mon_stack,
			K_THREAD_STACK_SIZEOF(cpu_mon_stack),
			cpu_mon_thread_entry, NULL, NULL, NULL,
			CPU_MON_PRIO, 0, K_NO_WAIT);
